...
- `motor`: автоматично спаровується з controller через зашифрований ESP-NOW і керує MX1508;
...
Телефони працюють з controller через Wi-Fi/WebSocket, а радіолінія між платами працює окремими компактними ESP-NOW пакетами кожні 50 мс. Controller передає підтверджений стан усім відкритим браузерам кожні 100 мс, тому вони синхронні. Плати автоматично знаходять одна одну, MAC-адреси вручну вводити не потрібно. Команди й статус після спарювання зашифровані спільним ключем із `include/config.h`.

Для двох вузлів у чистому полі mesh не потрібен. ESP8266-controller працює в `AP+STA`: AP обслуговує телефони, а STA-інтерфейс використовується тільки для ESP-NOW і не підключається до роутера. Motor працює в `STA`, але не приєднується до Wi-Fi мережі. Обидві плати зафіксовані на каналі 6. Інтернет і роутер не потрібні. 25–40 м реалістичні лише після польової перевірки конкретних антен, висоти встановлення та рівня завад.
...
Алгоритми не використовують `delay()`, тому сайт, WebSocket, ESP-NOW і аварійний STOP залишаються активними під час руху. Типовий програмний шлях команди: браузер → WebSocket → controller → найближчий 50-мс ESP-NOW пакет → motor. При втраті пакетів motor самостійно вимикає PWM через 1 секунду.
